...
# %%
welding_data.iloc[:,4:].corr()

# %%
bin=[15, 15, 25, 13, 17, 15, 35]
for index, value in enumerate(welding_data.iloc[:,4:].columns) :
    plt.figure(index)
    plt.hist(welding_data.iloc[:,4:][value], bins=bin[index], facecolor=(144/255,171/255,221/255), linewidth=3, edgecolor='black')
    plt.title(value)

# %%
n_welding_data = welding_data.iloc[:,6:]
n_welding_data

# %%
scaler = preprocessing.MinMaxScaler()
scaler.fit(n_welding_data)
scaled_data = scaler.fit_transform(n_welding_data)
...
# %%
train_data = torch.Tensor(scaled_data[:8470])
print(len(train_data))
test_data = torch.Tensor(scaled_data[8470:])
print(len(test_data))
...
for data in train_data :
    ...
    train_loss_chart.appkl bjend(loss.item())
